...
Singleton {
    ...
    property string gelbooruApiKey: "5661580f2d12d3efce5d81a5519f8c83d8404a6ce4721c3b5c03304a43d10484"
    property string gelbooruUserId: "1552772"
    ...
    function makeRequest(tags, nsfw=false, limit=20, page=1) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
                try {
                    // Guard against HTML error pages (e.g. Gelbooru hotlink/auth rejection)
                    const contentType = xhr.getResponseHeader("Content-Type") ?? ""
                    if (contentType.includes("text/html")) {
                        console.log("[Booru] Got HTML response instead of JSON — likely an auth or hotlink rejection (Content-Type: " + contentType + ")")
        ...
        try {
            ...
            if (currentProvider === "gelbooru") {
                xhr.setRequestHeader("Referer", "https://gelbooru.com")
            }
